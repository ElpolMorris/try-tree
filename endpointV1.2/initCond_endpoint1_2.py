...
from utils.verify_request_covid_series import verify_request_covid_series
    ...
    if request.method == 'POST':
        ...
        for sim in simulations:
            ...
            try:
                ...
                flagTimeInit = True
                if datetime.strptime(timeInit, '%Y-%m-%d') > datetime.strptime(timeEnd, '%Y-%m-%d'):
                    abort(400, description="timeInit must be lesser than timeEnd")
            except ValueError:
                abort(400, description="ERROR. Incorrect format or nonexistent timeInit or timeEnd")
            ...
            and flagTimeInit and flagTimeEnd):
                ...
                new_dict[sim] = fn.endpointResponse(apiRoute, dataset, compartments, scale, timeInit, timeEnd if apiRoute == "realDate" else timeInit, spatialSelection)
                status_code = 200
...
def get_initCond2(apiRoute):
    ...
    if request.method == 'POST':
        ...
        for index, requested_serie in enumerate(requested_covid_series.values()): 
            compartments, timeInit, timeEnd, scale, spatialSelection = itemgetter("compartments", "timeInit","timeEnd","scale","spatialSelection")(requested_serie)
            # validating series attributes
            verify_request_covid_series(compartments, timeInit, timeEnd, scale, spatialSelection)
            ...
            if compartments == "SEIRHVD":
                ...
                if scale == "States":  # statesUSA
                    ...
                    dataset = countiesUSA
            got_covid_series[series_names[index]] = fn.endpointResponse(apiRoute, dataset, compartments, scale, timeInit, timeEnd if apiRoute == "realData" else timeInit, spatialSelection)
    ...
    res = jsonify(got_covid_series,), 200
    return make_response(res)
